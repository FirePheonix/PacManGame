    ...
    if (pacmanPosition.x === x && pacmanPosition.y === y) {
      return (
        <div className="w-full h-full relative flex items-center justify-center">
          {/* Pac-Man with blinking animation */}
          <div className="w-5 h-5 bg-yellow-400 rounded-full relative animate-pulse transition-all duration-200 border border-yellow-500">
            {/* Pac-Man mouth - animated */}
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className="w-0 h-0 border-solid animate-bounce"
                   style={{
                     borderTop: '4px solid transparent',
                     borderBottom: '4px solid transparent',
                     borderLeft: '6px solid black',
                     animation: 'pacman-mouth 0.5s infinite alternate'
                   }}>
              </div>
            </div>
            {/* Eye */}
          ...
          <style jsx>{`
            @keyframes pacman-mouth {
              ...
              50% { transform: translate(-50%, -50%) rotate(0deg) scale(0.8); }
    ...
    <div className="bg-black p-4 rounded-lg border-4 border-blue-900 shadow-2xl">
      ...
      <style jsx global>{`
        @keyframes pacman-mouth {
          ...
          50% { transform: translate(-50%, -50%) rotate(0deg) scale(0.6); opacity: 0.8; }
